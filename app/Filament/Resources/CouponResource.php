            ...
            Forms\Components\Section::make('Coupon Details')->schema([
                ...
                Forms\Components\Grid::make(3)->schema([
                    Forms\Components\TextInput::make('value')
                        ->label(fn (Forms\Get $get): string => $get('type') === CouponType::Percentage->value
                            ? 'Discount (%)'
                            : 'Discount Amount (৳)')
                        ->numeric()
                        ->required()
                        ->minValue(0)
                        ->suffix(fn (Forms\Get $get): string => $get('type') === CouponType::Percentage->value ? '%' : '৳'),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('usage')
                    ->label('Used / Limit')
                    ->state(fn (Coupon $record): string => $record->used_count . ' / ' . ($record->usage_limit ?? '∞')),
